...
export default function AdminReservasPage() {
  ...
  async function cargar() {
    ...
    try {
      const data = await api.adminGetReservas(qs);
      setReservas(data);
    } catch {
  ...
  return (
    <div>
      ...
      <div className="filters card" style={{ marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            ...
            <tbody>
              {reservas.map((r) => (
                <tr key={r.id_reserva}>
                  ...
                  <td><span className={`badge ${ESTADO_LABEL[r.estado]}`}>{r.estado}</span></td>
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {r.estado !== 'CONFIRMADA' && (
                        <button className="btn btn-sm btn-primary" onClick={() => handleEstado(r.id_reserva, 'CONFIRMADA')}>Confirmar</button>
                      )}
                      {r.estado !== 'CANCELADA' && (
                        <button className="btn btn-sm btn-danger" onClick={() => handleEstado(r.id_reserva, 'CANCELADA')}>Cancelar</button>
                      )}
                    </div>
                  </td>
                </tr>
              ...
              {reservas.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--color-text-muted)' }}>Sin resultados</td></tr>
              )}
            </tbody>
          </table>
        </div>
