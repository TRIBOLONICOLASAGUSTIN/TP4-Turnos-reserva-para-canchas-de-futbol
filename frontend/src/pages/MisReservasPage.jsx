    ...
    <div className="mis-reservas">
      ...
      {proximas.length > 0 && (
        <>
          ...
          <div className="reservas-grid">
            {proximas.map((r) => (
              <ReservaCard key={r.id_reserva} reserva={r} onCancelar={handleCancelar} cancelando={cancelando} />
            ))}
          </div>
        </>
      )}
      ...
      {pasadas.length > 0 && (
        <>
          ...
          <div className="reservas-grid">
            {pasadas.map((r) => (
              <ReservaCard key={r.id_reserva} reserva={r} onCancelar={null} cancelando={null} />
            ))}
          </div>
        </>
      )}
...
function ReservaCard({ reserva, onCancelar, cancelando }) {
  const { label, cls } = ESTADO_LABEL[reserva.estado] || {};
